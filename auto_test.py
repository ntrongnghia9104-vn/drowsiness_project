...
if page == "1. Giới thiệu & Khám phá dữ liệu":
    ...
    * **MSSV:** 22T1020256
    ...
    Hệ thống giúp phát hiện sớm dấu hiệu giảm tỉnh táo qua luồng video bằng cách đo lường Tỷ lệ khung mắt (EAR). 
    Dự án ứng dụng mô hình Deep Learning **MediaPipe Face Mesh** của Google kết hợp với thuật toán đánh giá chuỗi thời gian để đưa ra cảnh báo chính xác, giảm thiểu tai nạn giao thông.
    """)
    st.divider()
    
    st.subheader("🔍 Mẫu dữ liệu thô (Raw Data)")
    st.markdown("Dưới đây là cấu trúc dữ liệu cơ bản được hệ thống theo dõi và ghi nhận liên tục theo từng khung hình (Frame) để làm cơ sở ra lệnh cảnh báo:")
    
    # Bảng dữ liệu đã được làm gọn, chỉ giữ lại các chỉ số cốt lõi
    mock_data = pd.DataFrame({
        'Khung hình (Frame)': [1, 2, 3, 4, 5],
        'Chỉ số EAR': [0.32, 0.31, 0.15, 0.12, 0.30],
        'Trạng thái ghi nhận': ['Mở mắt', 'Mở mắt', 'Nhắm mắt', 'Nhắm mắt', 'Mở mắt']
    })
